# Ford Blue/Green App - Installation Guide
...
The installer will:
1. Create the `ford-shop-dev` namespace
...
# Check pods
kubectl get pods -n ford-shop-dev

# View ArgoCD application details
argocd app get ford-bluegreen-app --grpc-web
```

## Instana Configuration

The installer creates the following secrets in the `ford-shop-dev` namespace:

- **backend-ford-instana-secret**: Contains agent key and endpoint for backend service
- **frontend-ford-instana-secret**: Contains EUM key and endpoint for frontend

These secrets are referenced by the application deployments and are **never committed to Git**.

## Blue/Green Deployment

This application uses Blue/Green deployment strategy:

- **Blue Environment**: Current production version
- **Green Environment**: New version being deployed
- **Traffic Switch**: Instant cutover from blue to green

Monitor deployments in ArgoCD to see the blue/green strategy in action.
...
kubectl delete application ford-bluegreen-app -n argocd-namespace

# Delete namespace (this will delete all resources including secrets)
kubectl delete namespace ford-shop-dev
```
...
# Force sync
argocd app sync ford-bluegreen-app --grpc-web
```
...
```bash
# Check pod logs
kubectl logs -n ford-shop-dev <pod-name>

# Check if secrets exist
kubectl get secrets -n ford-shop-dev
```
